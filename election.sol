...
contract Election {
    ...
    event ElectionVotes(string candidateName, uint voteCount);
    ...
    function vote(uint voteIndex) public {
        ...
        emit ElectionVotes(candidates[voteIndex].name, candidates[voteIndex].voteCount);
    }

    function end() ownerOnly public {
        //announce each candidates results
        for(uint i=0; i < candidates.length; i++) {
            emit ElectionVotes(candidates[i].name, candidates[i].voteCount);
        }
